...
- 财务数据按 **as-of 规则**对齐：只有 `ann_date <= as_of` 的财务快照参与计算；
  `ann_date` 优先使用真实 `pubDate`，缺失时才按配置推算并标记 `ann_date_is_approx`。
- 估值价格使用**当日不复权收盘价 × 点时总股本**；后复权价格（不复权价 × 复权因子本地换算，
  见 `docs/adr/0002`）只用于收益、动量和均线，杜绝估值与收益口径混用。
...
- 信号日固定为月末最后一个交易日，成交为其后第一个可交易日；批次按代码稳定排序，
  每批最多 20 个标的、单行业最多 25% 权重，剩余资金持有现金。
- 成交必须有逐交易日停牌/涨跌停状态；买入遇停牌或涨停、卖出遇停牌或跌停时视为未成交，
  不用未来价格替代；容量、单边成本和滑点统一由 `[portfolio]` 配置。
...
- 财务优先使用真实 `pubDate`；缺失时才按 `ann_date_approx_days` 推算并标记
  `ann_date_is_approx`；严格点时 `require_real_ann_date = true` 会阻断近似记录；
- 分位、动量、均线等所有因子一律以 as-of 日的可用数据计算；
- 已知的 as-of 限制（业绩预告仅覆盖部分公司、源缺少真实公告日时会阻断）须在报告的数据质量页声明。

回测报告额外输出成本后组合月均收益、最大回撤、年化波动率、累计换手、平均持仓数、现金比例、
行业暴露、固定切分样本外结果，以及去掉质量、恢复和价格辅信号的消融结果。

## 9. 已知精度限制（必须声明）

1. **公告日缺失**：严格流程拒绝只有推算公告日的财务记录；研究调试若关闭该门禁，报告必须声明
   近似公告日带来的 as-of 精度损失；
2. **北向资金缺失**：日频北向数据 2024-08-19 起停止披露，任何资金面判断不得使用北向信号；
3. **乐咕接口停更**：历史 PE/PB 分位接口已停更，全部估值分位自算（`docs/adr/0001`）；
4. **复权口径**：估值统一不复权 × 点时总股本，收益/动量/均线才使用本地复权因子（`docs/adr/0002`）；
